...
def do(building_configuration, target_curve):
    best_solution_matrix_rounded, metrics = ScheduleMaker.get_matrix_solution(building_configuration, target_curve)
    ...
    devices_outside_comfort_hourly = EvaluationDiagrams.count_devices_outside_comfort_hourly(
        best_solution_matrix_rounded, building_configuration.comfort_matrix)
    ...
    augmented_solution_headers = np.array(['Ap. id', 'Dev. id', 'H0', 'H1', 'H2', 'H3', 'H4', 'H5', 'H6', 'H7', 'H8', 'H9',
                                  ...
                                  'H22', 'H23'])
    augmented_solution = np.hstack((row_identifiers, best_solution_matrix_rounded))
    ...
    print("Running time: " + str(metrics["avg_running_time"]))
        ...
        'result': [
            {
                ...
                'data':
                    {
                        'rows': [["Running time (s):", FormattingUtils.seconds_to_formatted_hour_minute_second(metrics["avg_running_time"])],
                                 ["Distance (kWh):", round(metrics["avg_diff_kwh"], 2)],
                                 ["Discomfort coeff.:", metrics["avg_comfort"]]],
                        'header': [],
